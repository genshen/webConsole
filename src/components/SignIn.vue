    ...
    return {
      ...
      formRule: {
        fullHost: [
          {
            required: true,
            message: this.$t("signin.form_fullhost_required"),
            trigger: "blur"
          },
        ...
        username: [
          {
            required: true,
            message: this.$t("signin.form_username_required"),
            trigger: "blur"
          }
        ],
        password: []
    ...
    handleSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          let self = this;
          Utils.axiosInstance
            .post(Utils.loadUrl(apiRouters.router.sign_in), {
              ...
              try {
                if (!response.data || response.data.has_error) {
                  ...
                  switch (response.data.message) {
                    ...
                    case 1:
                      self.$Message.error(
                        self.$t("signin.form_error_passport")
                      );
                      break;
                    case 2:
                      self.$Message.error(
                        self.$t("signin.form_error_ssh_login")
                      );
                      break;
                  }
                } else {
                  if (!response.data.addition) {
                    self.$Loading.error();
                    self.$Message.error(
                      self.$t("signin.form_error_remote_server")
                    );
                  } else {
                    self.$Loading.finish();
