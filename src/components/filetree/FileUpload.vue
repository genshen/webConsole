<style scoped>
  ...
  .file-list-item, .file-uploading-list, .file-upload-container{
...
<template>
  <div>
    ...
    <div class="file-list-container">
      <ul>
        <li class="file-list-item" v-for="(file_item, index) in sortedFileList" :key="index"
            @click="onGridFileItemClicked(file_item)" v-on:dblclick="onGridFileItemDoubleClicked(file_item)">
          <div class="file-icon">
            <Icon v-if="file_item.is_dir" type="android-folder" size="32"></Icon>
            <Icon v-else type="document" size="32"></Icon>
          </div>
          <span class="file-list-filename">{{ file_item.title }}</span>
        </li>
        <li class="file-uploading-list" v-for="(item, index) in uploadList" :key="index">
          <template v-if="item.status !== 'finished'">
            <Progress v-if="item.showProgress" :percent="item.percentage" hide-info></Progress>
          </template>
        </li>
        <li class="file-upload-container">
          <Upload ref="upload_component" multiple type="drag"
            ...
            <div class="file-icon">
              <Icon type="android-upload" size="30"></Icon> <!--style="color: #2196f3"--->
            </div>
...
export default {
  ...
  computed: {
    sftp_upload_action () {
      ...
      if (_t) {
        return Utils.loadUrl(apiRouters.router.sftp_upload,
          stringFormat.format(apiRouters.params.sftp_upload, _t, this.cid, this.file_grid_list.current_path))
      }
      return ''
    },
    sortedFileList () {
  ...
  methods: {
      ...
      if (!fileItem.is_dir) {
        ...
        if (_t) {
          window.open(Utils.loadUrl(apiRouters.router.sftp_dl,
            stringFormat.format(apiRouters.params.sftp_dl, _t, this.cid, path)), '_self', false)
        }
      }
    },
    ...
    onPath (path) { // when the path is changed // todo add cache
      ...
      if (this.file_uploading) { // if it is uploading.
        this.$Message.warning(this.$t('console.file_transfer.wait_for_unload_finish'))
        return
      }
      const _t = sessionStorage.getItem(Config.jwt.tokenName)
      if (_t) {
        Utils.axiosInstance.get(Utils.loadUrl(apiRouters.router.sftp_ls,
          stringFormat.format(apiRouters.params.sftp_ls, _t, this.cid, true, path)), {}).then((response) => {
          try {
            if (!response.data || response.data.has_error) {
              ...
              messages.forEach((ele) => {
                children.push({title: ele.name, is_dir: ele.is_dir, path: ele.path}) // is_dir: maybe directory or file.
              })
    ...
    onBeforeUpload () { // if it is uploading.
    ...
    onUploadProgress (event, file) {
      ...
      if (per >= 100) {
        this.upload_status = this.$t('console.file_transfer.upload_status_pending')
      } else {
        this.upload_status = this.$t('console.file_transfer.upload_status_uploading', {percent: event.percent.toFixed(1)})
      }
    },
