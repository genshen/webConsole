...
util.config = {
    Domain: env === 'development' ? window.location.host :
        env === 'production' ? 'console.hpc.gensh.me' : window.location.host, //todo
    vpnParame: ',DanaInfo=console.hpc.gensh.me'
}

util.loadUrl = function (url) {
    if (env !== 'development' && window.location.host !== util.config.Domain) {
